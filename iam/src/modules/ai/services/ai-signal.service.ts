...
import { extractSignalJsonEnvelope, logDuration, mapSymbol } from 'src/shared/helper';
import { CalendarRepository } from 'src/modules/data/database/repositories/calendar.repository';
import { TradeSignal } from 'src/modules/data/database/schema/trade-signal.schema';
...
async analyzeAndCreateSignals(symbols: string[], language: string, timeframe: '5m'|'15m'|'1h'|'4h'|'1d' = '5m', userPrompt: string): Promise<{ responseText: string; prompt: string; systemPrompt:string }> {
    let sym;
    if (symbols.length >= 1) {
      sym = symbols[0];
    }
...
`;
    ...
    //return responseText;
    return { responseText, prompt, systemPrompt: systemPromptG5 };
  }
  ...
  private async deepseekChat(prompt: string, systemPrompt : string): Promise<string> {
    const baseURL = process.env.DEEPSEEK_BASE_URL || 'https://api.deepseek.com';
    const model   = process.env.DEEPSEEK_MODEL    || 'deepseek-chat';
    const temp    = process.env.DEEPSEEK_TEMPERATURE ? Number(process.env.DEEPSEEK_TEMPERATURE) : 0.2;
    ...
    const res = await axios.post(url, payload, {
      ...
      timeout: Number(process.env.DEEPSEEK_TIMEOUT_MS || 40_000)
    });
  ...
  /**
   * One-shot: produce prompt with analyzeAndCreateSignals, then ask DeepSeek with the SAME prompt,
   * extract NABZAR envelope, and persist TradeSignal with DeepSeek as the source.
   * Returns both the DeepSeek raw text and the prompt used.
   */
  async generateDeepseekSignalAndSave(
    fullPrompt: string,
    systemPrompt: string,
  ): Promise<{ deepseekText: string; fullPrompt: string; saved: boolean }> {

    
    // 2) Call DeepSeek using *the same* fullPrompt
    const deepseekText = await this.deepseekChat(fullPrompt, systemPrompt);

    // 3) Parse NABZAR envelope from DeepSeek output
    const sig = extractSignalJsonEnvelope(deepseekText);
    if (!sig) {
      this.logger.warn(`[DeepSeek] No NABZAR_SIGNAL_JSON found; not saving.`);
      return { deepseekText, fullPrompt, saved: false };
    }

    // 4) Persist TradeSignal with DeepSeek source
    const model = process.env.DEEPSEEK_MODEL || 'deepseek-chat';
    const nowSec = Math.floor(Date.now() / 1000);

    const doc: Partial<TradeSignal> = {
      symbol: sig.symbol,
      timeframe: sig.timeframe as TF,
      side: sig.action,
      entry: sig.entry,
      targets: sig.targets || [],
      stop: sig.stop ?? null,
      ...
      source: `deepseek-${model}-Automated`,
      status: 'open',
    ...
    try {
      ...
      this.logger.log(`[DeepSeek] saved open signal.`);
      return { deepseekText, fullPrompt, saved: true };
    } catch (e: any) {
      this.logger.error(`[DeepSeek] save failed: ${e?.message || e}`);
      return { deepseekText, fullPrompt, saved: false };
    }
  }
